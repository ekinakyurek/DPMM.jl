@testset "parallel algorithms" begin
    ...
    labels = fit(X; algorithm=CollapsedAlgorithm, quasi=true, T=100, ncpu=3)
    @info "Testing Direct Algorithm with 2 worker"
    labels = fit(X; algorithm=DirectAlgorithm, T=100, ncpu=3)
    @info "Testing Quasi-Direct Algorithm with 2 worker"
    labels = fit(X; algorithm=DirectAlgorithm, quasi=true, T=100, ncpu=3)
    @info "Testing Split-Merge Algorithm with 2 workers"
    labels = fit(X; algorithm=SplitMergeAlgorithm,T=100, ncpu=3)
    @info "Testing Split-Merge, without merge, Algorithm with 2 workers"
    labels = fit(X; algorithm=SplitMergeAlgorithm, merge=true, T=100, ncpu=3)
    @test true
end
