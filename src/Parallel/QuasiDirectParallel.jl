...
@inline quasi_direct_gibbs_parallel!(labels, clusters) =
    quasi_direct_parallel!(Main.model,Main.X,localindices(labels),labels,clusters,Main.empty_cluster)


function quasi_direct_gibbs_parallel!(model, X, clusters, labels::SharedArray; observables=nothing, T=10)
    for t=1:T
